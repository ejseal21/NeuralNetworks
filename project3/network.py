...
class Network():
    ...
    def backward(self, y):
        ...
        for i in range(len(self.layers)-1, -1, -1):
            d_upstream, d_wts, d_b = self.layers[i].backward(d_upstream, y)
...
class ConvNet4AccelExtension(Network):
    '''
    Makes a ConvNet4 network with the following layers: Conv2D -> MaxPooling2D -> Dense -> Dense
    ...
    '''
    def __init__(self, input_shape=(3, 32, 32), n_kers=(32,), ker_sz=(7,), dense_interior_units=(100,),
                 pooling_sizes=(2,), pooling_strides=(2,), n_classes=10, wt_scale=1e-2, reg=0, verbose=True):
        '''
        Parameters:
        -----------
        input_shape: tuple. Shape of a SINGLE input sample (no mini-batch). By default: (n_chans, img_y, img_x)
        ...
        self.layers.append(layer.Dense(len(self.layers), 'DenseRelu2', dense_interior_units[0], self.layers[-1].get_units(), wt_scale, 'relu', reg, verbose))
